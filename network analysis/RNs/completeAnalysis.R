...
# This script will apply the Relevance Network method on all non-randomized and randomized files, 
...
for(i in 1:nFiles){
  ...
  #applying the RNs method to the non-randomized data
  nonRandomGraph <- as.matrix((nonRandomCor > threshold) - Diagonal(dim(nonRandomCor)[1]))
  
  rownames(nonRandomGraph) <- colnames(nonRandomGraph) <- newMarkerNames
  nonRandomResult <- nonRandomGraph;
  
  #applying the glasso method to the randomized data
  randomGraph <- as.matrix((randomCor > threshold) - Diagonal(dim(nonRandomCor)[1]))
...
#### saving ####
write.csv(summaryStats, file = '../Results/RNs/completeAnalysisResults.csv', row.names = FALSE, quote = FALSE)
save(nonRandomResult, randomResults, summaryStats, file = '../Results/RNs/completeAnalysisResults.RData')
