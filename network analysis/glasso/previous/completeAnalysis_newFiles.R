...
nonRandomFile <- 'PBMC_nonrandomized.fcs'
randomFiles <- setdiff(dir('../../Data/randomized_new/', pattern = '.fcs'), nonRandomFile)
...
for(i in 1:nFiles){
  ...
  nonRandom <- read.FCS(file.path('../../Data/randomized_new', nonRandomFile))
  ...
  #reading random data
  random <- read.FCS(file.path('../../Data/randomized_new', randomFiles[i]))
  ...
  nonRandomResults <- nonRandomGraph;
...
#### saving ####
write.csv(summaryStats, file = '../../Results/glasso/completeAnalysisResults_newFiles.csv', row.names = FALSE, quote = FALSE)
save(nonRandomResults, randomResults, summaryStats, file = '../../Results/glasso/completeAnalysisResults_newFiles.RData')
